...
// Para FormData: NÃO incluir Content-Type (o browser seta o boundary automaticamente)
...
// Atribui cor estável baseada no id do grupo
...
function CreateTeamModal({ onClose, onCreate }) {
  ...
  const handle = async () => {
    ...
    try {
      ...
      // Upload de foto/banner se necessário
      if (avatarImage || coverImage) {
        const fd = new FormData();
        if (avatarImage) fd.append('photo', dataURLtoBlob(avatarImage), 'photo.png');
        if (coverImage) fd.append('banner', dataURLtoBlob(coverImage), 'banner.png');
      ...
      onCreate({ ...group, color, cover: coverImage, avatar: avatarImage, members: [] });
      onClose();
    } catch (err) {
        ...
        <div style={{ position: 'relative', borderRadius: 12, overflow: 'hidden', height: 120 }}>
          {coverImage ? (
            ...
            <div
              ...
              style={{
                ...
                cursor: 'pointer', gap: 6, transition: 'background 0.15s',
              }}
              onMouseEnter={e => e.currentTarget.style.background = '#e8e4de'}
              onMouseLeave={e => e.currentTarget.style.background = '#f0ede8'}
            >
          ...
          {coverImage && (
            <div style={{ position: 'absolute', bottom: 8, right: 8, display: 'flex', gap: 6 }}>
              <button onClick={() => coverRef.current.click()} style={{ background: 'rgba(0,0,0,0.5)', color: '#fff', border: 'none', borderRadius: 7, padding: '5px 10px', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, backdropFilter: 'blur(4px)' }}>Trocar</button>
              <button onClick={() => setCoverImage(null)} style={{ background: 'rgba(0,0,0,0.5)', color: '#fff', border: 'none', borderRadius: 7, padding: '5px 10px', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>✕ Remover</button>
            </div>
          )}
    ...
    <Modal title="Convidar pessoa" onClose={onClose}>
      <div style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          autoFocus
          style={{
            flex: 1, border: '1.5px solid #e2ddd6', borderRadius: 10,
            padding: '10px 13px', fontSize: 14, fontFamily: 'inherit',
            outline: 'none', color: '#1a1814', background: '#faf9f7',
          }}
          onFocus={e => e.target.style.borderColor = '#2c2a26'}
      ...
      {found && (
        <div style={{
          ...
          background: '#faf9f7', borderRadius: 12, padding: '12px 14px',
          border: '1.5px solid #e2ddd6',
        }}>
          <div style={{
            width: 40, height: 40, borderRadius: '50%', overflow: 'hidden',
            background: '#e8e4de', flexShrink: 0,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            {found.avatar_url
...
function TeamDetail({ team, onBack, onUpdate, onDelete }) {
  ...
  const changeCover = async (e) => {
    ...
    await fetch(`${API}/groups/${team.id}/`, {
      method: 'PATCH',
      headers: getAuthHeadersFormData(),
      body: fd,
    });
    onUpdate({ ...team, cover: url, color, members });
  };
  ...
  const changeAvatar = async (e) => {
    ...
    onUpdate({ ...team, avatar: url, color, members });
  };

  const hasCoverImage = team.cover || team.banner_url;
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 20 }}>
          <div>
            {editingName ? (
              ...
              <h1 onDoubleClick={() => setEditingName(true)} style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#1a1814', letterSpacing: '-0.4px', cursor: 'text' }} title="Duplo clique para editar">{team.name}</h1>
            )}
          ...
          <div style={{ maxWidth: 520 }}>
            <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.05)', padding: '24px' }}>
              ...
              <div style={{ marginTop: 20, padding: '14px 16px', background: color.bg, borderRadius: 10 }}>
                <p style={{ margin: 0, fontSize: 12, color: color.text, fontWeight: 600 }}>
                  Dica: você também pode convidar pessoas diretamente pela aba Membros buscando pelo username delas.
                </p>
              </div>
            </div>
          </div>
        ...
        {(team.members || []).length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            {(team.members || []).length > 5 && (
              <div style={{ marginLeft: -8, width: 26, height: 26, borderRadius: '50%', background: '#f0ede8', color: '#7a7570', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, border: '2px solid #fff', zIndex: 0 }}>+{(team.members || []).length - 5}</div>
            )}
          </div>
        )}
      </div>
...
export default function TeamsView() {
  ...
  const handleCreate = (group) => {
    setTeams(prev => [...prev, { ...group, color: group.color || getColorForGroup(group.id) }]);
  };
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32 }}>
          <div>
            ...
            <p style={{ margin: '5px 0 0', fontSize: 14, color: '#a09d97' }}>
              {loading ? 'Carregando…' : teams.length === 0
